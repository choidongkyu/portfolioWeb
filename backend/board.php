<?php
    class Board {
        ...
        public function getNext(){
            //모든 id를 내림차순으로 정렬
            $sql = "select board_id from t_board order by board_id desc";
            $res = $this->db->query($sql);
        ...
        public function write($board_title, $board_user_id, $board_content){
            ...
            $sql = "insert into t_board (board_id, board_title, board_user_id, board_date, board_content, board_available) 
        ...
        public function nextPage($pageNumber) {
            ...
            //board id 기준으로 내림차순으로 정렬 후 최대 10개만 리스트 가져옴
            $sql = "select * from t_board where board_id < '$limit_num' and board_available = 1";
            $res = $this->db->query($sql);
            //결과가 하나라도 존재 한다면
        ...
        public function getBoard($board_id) {
            $sql = "select * from t_board where board_id='$board_id'";
            $res = $this->db->query($sql);
            //결과가 하나라도 존재 한다면
        ...
        public function update($board_id, $board_title, $board_content) {
            $sql = "update t_board set board_title = '$board_title', board_content = '$board_content' where board_id='$board_id'";
            $res = $this->db->query($sql);
            return $res;
        }

        public function delete($board_id) {
            $sql = "delete from t_board where board_id=$board_id";
            $res = $this->db->query($sql);
            return $res;
        }
    }
